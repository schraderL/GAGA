...
GAGA.tree.plot2<-GAGA.tree.plot+geom_tiplab(aes(label=DisplayName,col=ifelse(grepl("GAGA",label),"GAGA","other")),size=1.5,show.legend=F)+scale_color_manual(name="",values = c("black","grey40"))+geom_rootedge(.005,size=.2)
```
...
for (tribe in tribes){
  if (tribeCount[tribe]>4){
    GAGA.tree.plot3<-GAGA.tree.plot3+geom_cladelabel(node=TribeNodes[tribe],label= tribe,align = T, offset = .086, col=c(cladeColor[tribe],"black"),barsize = 1,angle = 90,fontsize = 1.5,offset.text = .004,hjust = 'center')+xlim_tree(.1)
  }else{
    GAGA.tree.plot3<-GAGA.tree.plot3+geom_cladelabel(node=TribeNodes[tribe],label= "",align = T, offset = .086, col=cladeColor[tribe],barsize = 1)+xlim_tree(.1)
  }
}
...
```{R message=FALSE, warning=FALSE, include=FALSE}
GAGA.tree.plot.heatmap<-gheatmap(GAGA.tree.plot3+ylim(0,165), ddHeatmap[,c(4:7)], offset=.1, colnames=T, legend_title="Tech",  width = .15,colnames_position = "top", colnames_offset_y = 1,colnames_angle=90,hjust=0,font.size = 2.5)+
...
GAGA.tree.plot.heatmap.RNA<-gheatmap(GAGA.tree.plot3+ylim(0,165), ddHeatmapRNA[,c(15,16,17,10,14)], offset=.1, colnames=T, legend_title="Tech",  width = .15,colnames_position = "top", colnames_offset_y = 1,colnames_angle=90,hjust=0,font.size = 1.5)+
...
GAGA.GenomeSize2<-GAGA.GenomeSize+
  ...
  xlab("Species")+
  theme_classic()+
  theme(legend.position = "bottom",axis.text.y=element_text(size=6.5,color=chromosomeLevel))+
