...
# Script to retrieve the differentially expressed genes per groups (already generated by get_DE_pergroup_subfamily.pl) for a list of HOGs

# usage: perl get_DE_results_for_hog_list.pl hogtable.tsv

# Inputs
my $inhog = "$ARGV[0]";
my $intablesdir = "/Users/joel/Dropbox/Joel/trabajo/GAGAproject/Analyses_functional_tables_enrich/GAGA_DE_queen_worker/Queen_worker_DE_summaries";
#my $intablesdir = "../Queen_worker_DE_apo_hogs/Queen_worker_DE_summaries"; # HOGs from Apoidea (ants + 8 apoidea)
#my $intablesdir = "../Queen_worker_DE_apo_hogs/Queen_worker_DE_summaries"; # HOGs from Hymenoptera (ants + 8 apoidea + other hymenoptera + Tcas and Dmel)
...
open (Results, ">", "$inhog\_GAGADE.tsv");
print Results "HOG\tAnts DE (75pc is 75pc of species, 50pcs is 50pc strict, and chisq are significant DE using chisquare test)\tAnts DE data (QueenDE/WorkerDE/Totalspecies  logFC)\tFormicoids DE\tFormicoids DE data (QueenDE/WorkerDE/Totalspecies  logFC)\tPoneroids DE\tPoneroids DE data (QueenDE/WorkerDE/Totalspecies  logFC)\tSubfamily DE\tSubfamily DE data (QueenDE/WorkerDE/Totalspecies logFC)\n";
open (Resultsa, ">", "$inhog\_addGAGADE.tsv");
...
while(<File>){
	...
	if (exists $dedata{$hog}){
		foreach my $group (sort keys %{$dedata{$hog}}) {
			...
			if (exists $dedata{$hog}{$group}{"75pc"}) { # First priority, most strict
				...
				} elsif ($group =~ /Poneroid/){
					$ponede = "$subl[6]\_$group\_75pc";
					$ponededata = "$subl[1]\/$subl[2]\/$subl[4] $subl[5]";
				} else{
			...
			} elsif (exists $dedata{$hog}{$group}{"chisq"}){ # Last priority, less strict
	...
	print Results "$hog\t$antsde\t$antsdedata\t$formde\t$formdedata\t$ponede\t$ponededata\t$subfde\t$subfdedata\n";
	print Resultsa "$line\t";
	print Resultsa "$hog\t$antsde\t$antsdedata\t$formde\t$formdedata\t$ponede\t$ponededata\t$subfde\t$subfdedata\n";
