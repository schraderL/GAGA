...
CONFIG_PATH="/home/projects/ku_00039/people/joeviz/stlfr_assembly/masurca/run_GAGA-0495/config_file.txt"
CMD_PATH="/services/tools/masurca/3.3.0/bin/masurca"
set -o pipefail
...
#}
export PATH="/services/tools/masurca/3.3.0/bin/../CA8/Linux-amd64/bin:/services/tools/masurca/3.3.0/bin:$PATH"
save PATH
export PERL5LIB=/services/tools/masurca/3.3.0/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=20
save NUM_THREADS
...
rename_filter_fastq 'pe' <(exec expand_fastq '/home/projects/ku_00039/people/joeviz/data/stLFR_data/GAGA-0495/split_reads.1.fq.gz.clean' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq '/home/projects/ku_00039/people/joeviz/data/stLFR_data/GAGA-0495/split_reads.2.fq.gz.clean' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'pe.renamed.fastq'
...
JF_SIZE=`ls -l *.fastq | awk '{n+=$5}END{s=int(n/50); if(s>2500000000)printf "%.0f",s;else print "2500000000";}'`
save JF_SIZE
perl -e '{if(int('$JF_SIZE')>2500000000){print "WARNING: JF_SIZE set too low, increasing JF_SIZE to at least '$JF_SIZE', this automatic increase may be not enough!\n"}}'
log Creating mer database for Quorum
awk '{print substr($0,1,200)}' pe.renamed.fastq | quorum_create_database -t 20 -s $JF_SIZE -b 7 -m 24 -q $((MIN_Q_CHAR + 5)) -o quorum_mer_db.jf.tmp /dev/stdin && mv quorum_mer_db.jf.tmp quorum_mer_db.jf
if [ 0 != 0 ]; then
...
quorum_error_correct_reads  -q $((MIN_Q_CHAR + 40)) --contaminant=/services/tools/masurca/3.3.0/bin/../share/adapter.jf -m 1 -s 1 -g 1 -a 3 -t 20 -w 10 -e 3 -M  quorum_mer_db.jf pe.renamed.fastq --no-discard -o pe.cor.tmp --verbose 1>quorum.err 2>&1 && mv pe.cor.tmp.fa pe.cor.fa || fail Error correction of PE reads failed. Check pe.cor.log.
...
grep --text -A 1 '^>pe' pe.linking.fa | grep --text -v '^\-\-' | sample_mate_pairs.pl $MAX_LINKING_MATES $NUM_LINKING_MATES 1 > pe.tmp && error_corrected2frg pe 500 50 2000000000 pe.tmp > pe.linking.frg.tmp && rm pe.tmp && mv pe.linking.frg.tmp pe.linking.frg
...
runCA -s runCA.spec stopAfter=consensusAfterUnitigger -p genome -d CA cgwErrorRate=0.15 doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   pe.linking.frg    1> runCA1.out 2>&1
...
runCA -s runCA.spec stopAfter=consensusAfterUnitigger -p genome -d CA cgwErrorRate=0.15 doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   pe.linking.frg    1> runCA2.out 2>&1 
log "Recomputing A-stat for super-reads"
recompute_astat_superreads_CA8.sh genome CA $PE_AVG_READ_LENGTH work1/readPlacementsInSuperReads.final.read.superRead.offset.ori.txt superReadSequences_shr.frg
fi
