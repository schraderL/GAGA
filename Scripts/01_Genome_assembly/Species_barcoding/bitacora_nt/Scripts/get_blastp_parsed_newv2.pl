...
# Modified for nucleotide search
...
while (<Blastfile>) {
	...
	push (@{$blast{$subline[1]}}, join("\t",$subline[0], $subline[10], $subline[3], $subline[13], $subline[12], $ini, $fin, $subline[2]));
...
foreach my $key (sort keys %blast) {
	...
	foreach my $blastresult (@{$blast{$key}}) {
		...
		my $hitblast = $key;
		my $filtro1 = ($subline[4]*1)/15; ## Modified from 2/3. UCEs joined, or exons from genes
