...
my $maxorlength = "180"; # Maximum average length, it will print a warning with the sequences larger than this length
...
my $outgffall = "$gagaid\_"."$prefix"."\_CSP_renamed_all.gff3"; 
my $outprotall = "$gagaid\_"."$prefix"."\_CSP_renamed_all"; 

my $outgff = "$gagaid\_"."$prefix"."\_CSP_renamed_complete.gff3"; 
my $outprot = "$gagaid\_"."$prefix"."\_CSP_renamed_complete"; 
...
my %dmelhit;
my %anthit;
my $blastirfile = "$prefix".".pep.fasta.CSPblast.txt";
...
#		next unless ($subline[10] <= "1e-20"); # High evalue to keep Dmel orthologs
	...
	if ($subline[1] =~ /Cflo/ || $subline[1] =~ /Hsal/){
...
open (Results, ">", "$gagaid\_CSP.txt");
print Results "GAGA ID\tSpecies name\tGene name\tScaffold location\tCSP name\tCSP subfamily\tLength\tExon number\tBlast hit\tDomain\tDmel best hit\tAnt best hit\tStatus Complete Partial Pseudogene\tFragment CSP\n";

open (Resultssum, ">", "$gagaid\_CSP_summary.txt");
print Resultssum "GAGA ID\tSpecies name\tCSP total number\tComplete\tComplete with domain\tPartial\tPartial with domain\tPartial longer 250aa\tPseudogene\tPseudogene with domain\tPseudogene longer 250aa\tLength lower than 150aa\tLength lower than 100aa\tLength lower than 50aa\t";
#print Resultssum "PPK16 count\tPPK28 count\t";
print Resultssum "CSP complete + partial with domain\tFinal retained CSP\tFinal retained complete CSP (complete + partial >120aa)\tFinal retained complete de novo annot\tFinal retained partial CSP\tFinal retained partial de novo annot\tFinal retained pseudogene CSP\tFinal retained complete CSP\tFinal retained incomplete or pseudogene CSP\tFragment CSP partial\tFragment CSP pseudogenes\n";

my $irnum = "1"; # Starting to count CSP in 1
my $fragornum = "1";
...
#				}	
		...
		if ($discard =~ /Yes/){
			...
			if ($deleteproteinsl50 =~ /$prot /){
				...
				$subfcount{$subfamily}++;
				if ($subfamily =~ /CSP$/){ # Not gonna do anything here
...
#		}
		...
		if ($deleteproteins =~ /$genename /){
			...
			if ($hstatus{$genename} =~ /Complete/){
				...
				if ($nnamef =~ /$gagashort{$gagaid}\S*CSP/){
					$fircount++;
				}
			} elsif ($hstatus{$genename} =~ /Partial/){
				$fpar++;
