...
my $evalue = $ARGV[3];
my $minlengthcut = $ARGV[2]; ## Minimum positions required to trim a protein (i.e. blast hits starting in position 10 will report the full sequence instead of trimming the first 10 aa)
...
foreach my $key (sort keys %blast) {
	...
	foreach my $blastresult (@{$blast{$key}}) {
		...
		my $lini = ""; # New lines to edit the start and end position to allow overlapping positions
		my $lend = "";
		my $maxend = $subline[3] - 50;
		if (int($subline[5]) <= 50){
			$lini = "1";
		} else {
			$lini = int($subline[5]) + 20;
		}	
		if (int($subline[6]) >= $maxend ){
			$lend = $subline[3];
		} else {
			$lend = int($subline[6]) - 20;
		}	
		##	
