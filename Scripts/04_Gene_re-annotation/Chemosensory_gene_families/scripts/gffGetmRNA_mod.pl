...
sub printmrnas() {
	...
	if ($sequence{$chr}){
		...
		foreach my $exon (@{$txs{$chr}{$txid}}){
			...
			my $seqpart = "";
			if ($exon->[4] > length($sequence{$chr})){
				print "WARNING in gffGetmRNA.pl: Coordinate of $txid out of sequence range. $exon->[4] >= " . length($sequence{$chr}) . "  USING AS FINAL POSITION THE END OF THE CHROMOSOME\n";
				$seqpart = lc(substr($sequence{$chr}, $exon->[3]-1, length($sequence{$chr}) - $exon->[3] + 1));
			} else {
				$seqpart = lc(substr($sequence{$chr}, $exon->[3]-1, $exon->[4] - $exon->[3] + 1));
			}
