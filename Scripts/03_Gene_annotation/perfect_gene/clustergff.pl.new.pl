...
if ($type eq "1" ){
	...
	`perl $Bin/cluster/run_cluster_lsp.pl  $file2 $dirname/$basename.cds.len $run $cutoff $cpu`;
}else{
	`perl $Bin/get_genewise_score.pl $type >$dirname/$basename.genewise.score `;
	`perl $Bin/cluster/run_cluster_lsp.pl $file2 $dirname/$basename.genewise.score $run $cutoff $cpu `;
}
