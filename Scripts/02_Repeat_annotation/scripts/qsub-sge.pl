...
$Queue ||= "st.q";
$Pro_code ||= "P18Z10200N0160"; #"F16ZQSB1SY2933";
$Interval ||= 60;
$Lines ||= 1;
$Maxjob ||= 80;
$Convert ||= 'yes';
...
while (@Shell) {
	...
	my $job_cmd = "/opt/gridengine/bin/lx-amd64/qsub -clear -cwd ";  ##-S /bin/sh ";  ## -l h_vmem=16G,s_core=8
	if (-d "/ifshk1")
	{
...
#	die "no valid resource\n" if(@resources<1);#hezengquan 2009-10-19
#	my $part_resource;#hezengquan 2009-10-19
...
sub run_count {
	...
	my $qstat_result = `/opt/gridengine/bin/lx-amd64/qstat -u $user`;
		...
		if (exists $all_p->{$job_field[0]}){
			...
			if ( !exists $died{$node_name} && ($job_field[4] eq "qw" || $job_field[4] eq "r" || $job_field[4] eq "t" || $job_field[4] eq "s") ) { ##add  $job_field[4] eq "s" for the holded job by fangqi
				...
				if ((defined $getmem) && ($job_field[4] eq "r")){### get mem ;  add by nixiaoming nixiaoming@genomics.cn
					my $jobinfo=`/opt/gridengine/bin/lx-amd64/qstat -j $job_field[0] 2>&1 |grep usage `;
					$$memlist{$all_p->{$job_field[0]}}=$jobinfo;
				}
			}else{
#				`qdel $job_field[0]`; #by fangqi, 20171001, for the holded jobs from yuhao monitor scripts
			}
		}
	}
...
##HOSTNAME                ARCH         NCPU NSOC NCOR NTHR  LOAD  MEMTOT  MEMUSE  SWAPTO  SWAPUS for 2018 version qhost
...
sub died_nodes{
	...
	foreach  (@lines) {
		...
		$died_p->{$node_name} = 1 if($t[6]=~/-/ || $t[7]=~/-/ || $t[8]=~/-/ || $t[9]=~/-/ || $t[10]=~/-/); #if($t[3]=~/-/ || $t[4]=~/-/ || $t[5]=~/-/ || $t[6]=~/-/ || $t[7]=~/-/);
	}
