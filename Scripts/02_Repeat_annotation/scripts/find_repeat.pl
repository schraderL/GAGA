...
my $species_name=$1 if ($seq_file_name=~/^(\S+)\.fasta/);
...
my $repeatmasker_para = "";
if(defined $Lib) {
	$repeatmasker_para = "-s -a  -inv  -nolow -no_is -norna -engine ncbi  -parallel 1 -lib $Lib ";
}else{
	$repeatmasker_para = "-s -a  -inv  -nolow -no_is -norna -engine ncbi  -parallel 1";
}
...
#my $proteinmask_para = "-noLowSimple -pvalue $Pvalue "; changed by hlj
my $proteinmask_para = "-engine ncbi -noLowSimple -pvalue $Pvalue ";
...
if (defined $Trf) {
	...
	close OUT;
	if(defined $Node){
		`$qsub_sge $QP_para --maxjob $Cpu --resource $Resource --node $Node $trf_shell_file; mv $trf_shell_file.*.qsub/*.dat $Outdir/$seq_file_name.cut/;` if ($Run eq "qsub");
	}
	else{`$qsub_sge $QP_para --maxjob $Cpu --resource $Resource  $trf_shell_file; mv $trf_shell_file.*.qsub/*.dat $Outdir/$seq_file_name.cut/;` if ($Run eq "qsub");}
...
if (defined $RepeatMasker) {
	...
	close OUT;
	if(defined $Node){
		`$qsub_sge $QP_para --maxjob $Cpu --resource $Resource --node $Node $repeatmasker_shell_file` if ($Run eq "qsub");
	}
	else{`$qsub_sge $QP_para --maxjob $Cpu --resource $Resource  $repeatmasker_shell_file` if ($Run eq "qsub");}
	`$multi_process -cpu $Cpu $repeatmasker_shell_file` if ($Run eq "multi");
	`find $Outdir/$seq_file_name.cut/ -name "*.out" ! -name '*.ori.out' | xargs cat > $Outdir/$species_name.$repeatmasker_style.RepeatMasker.out`;
...
if (defined $Proteinmask) {
	...
	close OUT;
	if(defined $Node){
		`$qsub_sge $QP_para --maxjob $Cpu --resource $Resource --node $Node $proteinmask_shell_file` if ($Run eq "qsub");
	}
	else{`$qsub_sge $QP_para --maxjob $Cpu --resource $Resource  $proteinmask_shell_file` if ($Run eq "qsub");}
