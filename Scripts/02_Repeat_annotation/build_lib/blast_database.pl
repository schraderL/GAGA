...
my ($Program,$Evalue,$Nr,$Nt,$Swissprot,$TrEMBL,$Cog,$Kegg,$TEdna,$TEpep,$Userdb,$Cutf,$Cpu,$Node,$Run,$Outdir,$Queue,$Pro_code);
my ($Verbose,$Help);
GetOptions(
	"program:s"=>\$Program,
	"evalue:s"=>\$Evalue,
...
my $blast = parse_config($config_file,"blastall")." -b 10000 -v 10000 -F F";;
...
my $blast_parser = parse_config($config_file,"blast_parser");
...
#run the shell file
if(defined $Node){
`$qsub_sge $QP_para --maxjob $Cpu  --node $Node $blast_shell_file` if ($Run eq "qsub");
}
else{`$qsub_sge $QP_para --maxjob $Cpu  $blast_shell_file` if ($Run eq "qsub");}
