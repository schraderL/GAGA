...
while (<IN>) {
	...
	close (F);
	print "cd /run/media/dell/storage1/User/xiongzj/GAGA_project/15.new_anntations/Gene_Re-annotations1/Final_annotations_output_toZijun/$id/CombinedEvidences1; mkdir Old; mv *.gz ./Old; nohup sh ../../bin/pipeline.sh $id $prefix & \n";
}
close (IN);
