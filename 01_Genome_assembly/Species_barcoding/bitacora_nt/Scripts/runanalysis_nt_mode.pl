...
#####
#
# Run first analysis, using blastn (and hmmer: NO) without a GFF file
#
#####
...
# Indexing genome
print "Indexing genome file $transcripts\n";
...
while(<File>){
	...
	my $line = $_;
	next if ($line !~ /\S+/);
	if ($line =~ /$transcripts\.nin/ || $line =~ /$transcripts\.nhr/ || $line =~ /$transcripts\.nsq/){
		$indexed++;
	}
...
if ($indexed == 3){
	...
	system ("makeblastdb -dbtype nucl -in $transcripts");
}
...
foreach my $chem (@chemosensory){
	print "\n----------------- Starting $chem Nt Identification and Annotation\n";	
#=head
	system ("mkdir -p $chem");
	# run blastp
	print "Doing $chem blastn\n";
	system ("cp $chemdir\/$chem\_db.fasta $chem\/$chem\_db.fasta");
	system ("blastn -query $chemdir\/$chem\_db\.fasta -db $transcripts -out $chem\/$name\_Vs$chem\_blastp\.outfmt6 -evalue $evalue -num_threads $threads -outfmt \"6 std qlen slen\" "); # Deprecated
	...
	system ("touch $chem\/hmmer\/$name\_$chem\.domtblout");
#=cut
