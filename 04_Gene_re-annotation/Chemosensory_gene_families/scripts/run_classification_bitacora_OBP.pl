...
my $orcompletelength = "100"; # Length to consider complete OR copies
my $minorlength = "60"; # Minimum length to retain partial ORs
my $maxorlength = "260"; # Maximum average length, it will print a warning with the sequences larger than this length
...
my $outgffall = "$gagaid\_"."$prefix"."\_OBP_renamed_all.gff3"; 
my $outprotall = "$gagaid\_"."$prefix"."\_OBP_renamed_all"; 

my $outgff = "$gagaid\_"."$prefix"."\_OBP_renamed_complete.gff3"; 
my $outprot = "$gagaid\_"."$prefix"."\_OBP_renamed_complete"; 
...
my %dmelhit;
my %anthit;
my $blastirfile = "$prefix".".pep.fasta.OBPblast.txt";
...
#		next unless ($subline[10] <= "1e-20"); # High evalue to keep Dmel orthologs
	...
	if ($subline[1] =~ /Cflo/ || $subline[1] =~ /Hsal/ || $subline[1] =~ /Sinv/ || $subline[1] =~ /Lhum/ || $subline[1] =~ /Mpha/ || $subline[1] =~ /Obir/){
...
# Reading venom allergen blast
my %venomhit;
my $blastvenomfile = "$prefix".".pep.fasta.OBPvenomallergenblast.txt";
open (File, "<", $blastvenomfile);
while(<File>){
	chomp;
	$line = $_;
	next if ($line !~ /\S+/);	
	...
	if (exists $venomhit{$subline[0]}){
		my @subl = split (/\t/, $venomhit{$subline[0]});
		if ($subline[10] < $subl[1]){
			$venomhit{$subline[0]} = "$subline[1]\t$subline[10]";
		}			
	} else {
		$venomhit{$subline[0]} = "$subline[1]\t$subline[10]";
	}
...
open (Results, ">", "$gagaid\_OBP.txt");
print Results "GAGA ID\tSpecies name\tGene name\tScaffold location\tOBP name\tOBP subfamily\tLength\tExon number\tBlast hit\tDomain\tDmel best hit\tAnt best hit\tVenom allergen hit\tStatus Complete Partial Pseudogene\tFragment OBP\n";

open (Resultssum, ">", "$gagaid\_OBP_summary.txt");
print Resultssum "GAGA ID\tSpecies name\tOBP total number\tComplete\tComplete with domain\tPartial\tPartial with domain\tPartial longer 250aa\tPseudogene\tPseudogene with domain\tPseudogene longer 250aa\tLength lower than 150aa\tLength lower than 100aa\tLength lower than 50aa\t";
print Resultssum "OBP59a count\tGp-9 count\t";
print Resultssum "OBP complete + partial with domain\tFinal retained OBP\tFinal retained complete OBP (complete + partial >120aa)\tFinal retained complete de novo annot\tFinal retained partial OBP\tFinal retained partial de novo annot\tFinal retained pseudogene OBP\tFinal retained complete OBP59a\tFinal retained incomplete or pseudogene OBP59a\tFragment OBP partial\tFragment OBP pseudogenes\n";

my $irnum = "1"; # Starting to count OBP in 1
my $fragornum = "1";
...
#my $snmp = "0"; my $snmp1 = "0"; my $snmp2 = "0"; my $snmp3 = "0"; my $group2 = "0"; my $group1 = "0"; my $emp = "0"; my $santa = "0"; my $dsb = "0"; 
			...
			if ($subl[1] > "1e-20"){
				if (exists $anthit{$prot}){
					...
					if ($ahit[1] < "1e-20"){
						if ($ahit[0] =~ /Obp59a/ || $ahit[0] =~ /OBP59a/){
							$subfamily = "OBP59a";
							$obp59a++; 
						} elsif ($ahit[0] =~ /Gp-9/ ){
#							$subfamily = "OBPGp-9";
...
#					} 
				...
				if (exists $anthit{$prot}){
					my @ahit = split (/\t/, $anthit{$prot});
					if ($ahit[1] > $subl[1]){ # Dmel shows better evalue
						if ($dmelblasthit =~ /Obp59a/ || $ahit[0] =~ /OBP59a/){
							$subfamily = "OBP59a";
							$obp59a++;
						} elsif ($ahit[0] =~ /Gp-9/ ){
							#$subfamily = "OBPGp-9";
...
#					$subfamily = "IR";
		...
		# Venom allergens
		my $venomblasthit = "None";
		if (exists $venomhit{$prot}){
			my @subl = split (/\t/, $venomhit{$prot});
			$venomblasthit = "$subl[0] $subl[1]"; 
		}
		...
		if (exists $interpro{$prot}){
			...
			if ($interpro{$prot} =~ /IPR006170/){ #  || $interpro{$prot} =~ /IPR036728/
				$ordomain = "OBP";
			...
			} elsif ($interpro{$prot} =~ /IPR003392/){ # NPC1 - patched domain
				$ordomain = "NPC1";
				$deleteproteins .= "$prot "; ### Delete complete protein identified with other domain
				$discard = "Yes";
				$deleteproteinsl50 .= "$prot ";				
			} else {
...
#				}
		...
		# Strict filter for the OBP putative mis-annotations venom allergen proteins, delete all proteins not hitting the OBP db. 
		if ($irblasthit =~ /None/ && $venomblasthit !~ /None/){
			$deleteproteins .= "$prot "; 
			$discard = "Yes";
			$deleteproteinsl50 .= "$prot ";
		}
...
#				}	
		...
		if ($discard =~ /Yes/){
			...
			if ($deleteproteinsl50 =~ /$prot /){
				...
				$subfcount{$subfamily}++;
				if ($subfamily =~ /OBP$/){ # Not gonna do anything here
...
#print Results "GAGA ID\tSpecies name\tGene name\tScaffold location\tIR/iGluR name\tIR/iGluR subfamily\tLength\tExon number\tBlast hit\tDomain\tDmel best hit\tAnt best hit\tStatus Complete Partial Pseudogene\tFragment GR\n";
		print Results "$gagaid\t$gagasp{$gagaid}\t$prot\t$scaf\-$coord\t$nname\t$subfamily\t$len\t$exonnumber{$prot}\t$irblasthit\t$ordomain\t$dmelblasthit\t$antblasthit\t$venomblasthit\t$status\t$discard\n";
...
#		}
		...
		if ($deleteproteins =~ /$genename /){
			...
			if ($hstatus{$genename} =~ /Complete/){
				...
				if ($nnamef =~ /$gagashort{$gagaid}\S*OBP59a/){
					$fircount++;
				}
			} elsif ($hstatus{$genename} =~ /Partial/){
				$fpar++;
