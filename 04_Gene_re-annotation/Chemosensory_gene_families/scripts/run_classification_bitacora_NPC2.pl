...
my $orcompletelength = "120"; # Length to consider complete OR copies
my $minorlength = "80"; # Minimum length to retain partial ORs
my $maxorlength = "200"; # Maximum average length, it will print a warning with the sequences larger than this length
...
my $outgffall = "$gagaid\_"."$prefix"."\_NPC2_renamed_all.gff3"; 
my $outprotall = "$gagaid\_"."$prefix"."\_NPC2_renamed_all"; 

my $outgff = "$gagaid\_"."$prefix"."\_NPC2_renamed_complete.gff3"; 
my $outprot = "$gagaid\_"."$prefix"."\_NPC2_renamed_complete"; 
...
my %dmelhit;
my %anthit;
my $blastirfile = "$prefix".".pep.fasta.NPC2blast.txt";
...
#		next unless ($subline[10] <= "1e-20"); # High evalue to keep Dmel orthologs
	...
	if ($subline[1] =~ /Cflo/ || $subline[1] =~ /Cjap/){
...
open (Results, ">", "$gagaid\_NPC2.txt");
print Results "GAGA ID\tSpecies name\tGene name\tScaffold location\tNPC2 name\tNPC2 subfamily\tLength\tExon number\tBlast hit\tDomain\tDmel best hit\tAnt best hit\tStatus Complete Partial Pseudogene\tFragment NPC2\n";

open (Resultssum, ">", "$gagaid\_NPC2_summary.txt");
print Resultssum "GAGA ID\tSpecies name\tNPC2 total number\tComplete\tComplete with domain\tPartial\tPartial with domain\tPartial longer 250aa\tPseudogene\tPseudogene with domain\tPseudogene longer 250aa\tLength lower than 150aa\tLength lower than 100aa\tLength lower than 50aa\t";
#print Resultssum "PPK16 count\tPPK28 count\t";
print Resultssum "NPC2 complete + partial with domain\tFinal retained NPC2\tFinal retained complete NPC2 (complete + partial >120aa)\tFinal retained complete de novo annot\tFinal retained partial NPC2\tFinal retained partial de novo annot\tFinal retained pseudogene NPC2\tFinal retained complete NPC2\tFinal retained incomplete or pseudogene NPC2\tFragment NPC2 partial\tFragment NPC2 pseudogenes\n";

my $irnum = "1"; # Starting to count NPC2 in 1
my $fragornum = "1";
...
#				}	
		...
		if ($discard =~ /Yes/){
			...
			if ($deleteproteinsl50 =~ /$prot /){
				...
				$subfcount{$subfamily}++;
				if ($subfamily =~ /NPC2_$/){ # Not gonna do anything here
...
#		}
		...
		if ($deleteproteins =~ /$genename /){
			...
			if ($hstatus{$genename} =~ /Complete/){
				...
				if ($nnamef =~ /$gagashort{$gagaid}\S*NPC2/){
					$fircount++;
				}
			} elsif ($hstatus{$genename} =~ /Partial/){
				$fpar++;
