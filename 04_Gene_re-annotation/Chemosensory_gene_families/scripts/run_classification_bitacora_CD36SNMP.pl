...
my $orcompletelength = "400"; # Length to consider complete CD36-SNMPs copies
my $minorlength = "200"; # Minimum length to retain partial CD36-SNMPs
my $maxorlength = "700"; # Maximum average length, it will print a warning with the sequences larger than this length
...
my $outgffall = "$gagaid\_"."$prefix"."\_CD36-SNMPs_renamed_all.gff3"; 
my $outprotall = "$gagaid\_"."$prefix"."\_CD36-SNMPs_renamed_all"; 

my $outgffir = "$gagaid\_"."$prefix"."\_SNMPs_renamed_all.gff3"; 
my $outprotir = "$gagaid\_"."$prefix"."\_SNMPs_renamed_all"; 

my $outgffiglur = "$gagaid\_"."$prefix"."\_SNMPs_renamed_complete.gff3"; 
my $outprotiglur = "$gagaid\_"."$prefix"."\_SNMPs_renamed_complete"; 
...
my %dmelhit;
my %anthit;
my $blastirfile = "$prefix".".pep.fasta.CD36blast.txt";
...
open (Results, ">", "$gagaid\_CD36_SNMPs.txt");
print Results "GAGA ID\tSpecies name\tGene name\tScaffold location\tCD36-SNMP name\tCD36-SNMP subfamily\tLength\tExon number\tBlast hit\tDomain\tDmel best hit\tAmel best hit\tStatus Complete Partial Pseudogene\tFragment CD36-SNMP\n";

open (Resultssum, ">", "$gagaid\_CD36_SNMPs_summary.txt");
print Resultssum "GAGA ID\tSpecies name\tCD36-SNMP total number\tComplete\tComplete with domain\tPartial\tPartial with domain\tPartial longer 250aa\tPseudogene\tPseudogene with domain\tPseudogene longer 250aa\tLength lower than 150aa\tLength lower than 100aa\tLength lower than 50aa\t";
print Resultssum "SNMP count\tSNMP1 count\tSNMP2 count\tSNMP3 count\tGroup2 count\tGroup1 count\tEmp count\tSantaMaria count\tDsb count\t";
print Resultssum "CD36 complete + partial with domain\tFinal retained CD36-SNMPs\tFinal retained complete CD36-SNMPs (complete + partial >350aa)\tFinal retained complete de novo annot\tFinal retained partial CD36-SNMPs\tFinal retained partial de novo annot\tFinal retained pseudogene CD36-SNMPs\tFinal retained complete SNMPs\tFinal retained incomplete or pseudogene SNMPs\tFragment CD36-SNMPs partial\tFragment CD36-SNMPs pseudogenes\n";
				...
				if (exists $anthit{$prot}){
					...
					if ($ahit[1] < "1e-30"){
						if ($ahit[0] =~ /SNMP1/ ){
							...
							$subfamily = "CD36Group1aEmp";
							$group1++; $emp++;
						} elsif ($ahit[0] =~ /Santa/ ){
							$subfamily = "CD36Group2b";
							$santa++; $group2++;
						} elsif ($ahit[0] =~ /Dsb/ ){
							$subfamily = "CD36Group1aDsb";
							$dsb++; $group1++;
						} elsif ($ahit[0] =~ /Crq/ ){
							$subfamily = "CD36Group2a";
							$group2++;
						} elsif ($ahit[0] =~ /Group2/ ){
							if ($ahit[0] =~ /Group2a/ ){
								$subfamily = "CD36Group2a";
							} elsif ($ahit[0] =~ /Group2b/ ){
								$subfamily = "CD36Group2b";
							} else {
								$subfamily = "CD36Group2";
							}
							$group2++;
						} elsif ($ahit[0] =~ /Group1/ ){
							if ($ahit[0] =~ /Group1a/ ){
								$subfamily = "CD36Group1a";
							} elsif ($ahit[0] =~ /Group1b/ ){
								$subfamily = "CD36Group1b";
							} else {
								$subfamily = "CD36Group1";
							}
							$group1++;
						}						
						...
						if ($ahit[1] > $subl[1]){ # Dmel shows better evalue
							if ($dmelblasthit =~ /SNMP1/ ){
								$subfamily = "SNMP1";
								$snmp1++; $snmp++;
							} if ($dmelblasthit =~ /SNMP2/ ){
								$subfamily = "SNMP2";
								$snmp2++; $snmp++;
							} elsif ($dmelblasthit =~ /SNMP3/ ){
								$subfamily = "SNMP3";
								$snmp3++; $snmp++;
							} elsif ($dmelblasthit =~ /Emp/ ){
								$subfamily = "CD36Group1aEmp";
								$group1++; $emp++;
							} elsif ($dmelblasthit =~ /Santa/ ){
								$subfamily = "CD36Group2b";
								$santa++; $group2++;
							} elsif ($dmelblasthit =~ /Dsb/ ){
								$subfamily = "CD36Group1aDsb";
								$dsb++; $group1++;
							} elsif ($dmelblasthit =~ /Crq/ ){
								$subfamily = "CD36Group2a";
								$group2++;
							} elsif ($dmelblasthit =~ /Group2/ ){
								if ($dmelblasthit =~ /Group2a/ ){
									$subfamily = "CD36Group2a";
								} elsif ($dmelblasthit =~ /Group2b/ ){
									$subfamily = "CD36Group2b";
								} else {
									$subfamily = "CD36Group2";
								}
								$group2++;
							} elsif ($dmelblasthit =~ /Group1/ ){
								if ($dmelblasthit =~ /Group1a/ ){
									$subfamily = "CD36Group1a";
								} elsif ($dmelblasthit =~ /Group1b/ ){
									$subfamily = "CD36Group1b";
								} else {
									$subfamily = "CD36Group1";
								}
...
#				}	
		...
		if ($discard =~ /Yes/){
			...
			if ($deleteproteinsl50 =~ /$prot /){
				...
				$subfcount{$subfamily}++;
				if ($subfamily =~ /IR$/){ # Not gonna do anything here
...
#					$nname = "$gagashort{$gagaid}"."frag$subfamily"."\.$subfcountfrag{$subfamily}"."$sufix";
					$nname = "$gagashort{$gagaid}"."frag$subfamily"."\_g$subfcount{$subfamily}"."$sufix"; # Change to number it for all complete and partial
				}
...
#				$fragornum++;
			...
			if ($subfamily =~ /IR$/){
				...
				$nname = "$gagashort{$gagaid}"."$subfamily"."\_g$subfcount{$subfamily}"."$sufix";
			}
