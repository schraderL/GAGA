...
print Results "GAGA ID\tSpecies name\tGene name\tOriginal name\tOR name\tLength\tExon number\tBlast blast hit\tDomain\tDmel function\tStatus Complete Partial Pseudogene\tFragment GR\n";
...
#print Resultssum "GAGA ID\tSpecies name\tOR total number\tComplete\tComplete with domain\tPartial\tPartial with domain\tPartial longer 350aa\tPseudogene\tPseudogene with domain\tPseudogene longer 350aa\tLength lower than 200aa\tLength lower than 100aa\tLength lower than 50aa\tOR count\tOrco\tOrco complete\tNine-exon ORs complete\tNine-exon ORs incomplete\tNine-exon ORs incomplete (length > 350aa)\tNine-exon ORs pseudogenes\tOR complete + partial with domain\tFinal retained ORs\tFinal retained complete ORs (complete + partial >350aa) \tFinal retained partial ORs\tFinal retained pseudogene ORs\tFinal retained complete 9-exon ORs\tFinal retained incomplete or pseudogene 9-exon ORs\tFragment ORs partial\tFragment ORs pseudogenes\n";
...
#				}
		...
		print Results "$gagaid\t$gagasp{$gagaid}\t$prot\t$sname\t$nname\t$len\t$exonnumber{$prot}\t$orhit\t$ordomain\t$dmelhit\t$status\t$discard\n";
...
#		next if ($deleteproteins =~ /$genename /); ### Delete genes that do not pass the filter
		next if ($deleteproteinsl50 =~ /$genename /); ### Delete from GFF all genes shorted than 50aa or mis-identification such as GR
