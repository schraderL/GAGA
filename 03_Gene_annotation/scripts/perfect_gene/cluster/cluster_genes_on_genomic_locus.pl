...
foreach my $chr (sort keys %Content) {
	...
	`perl $Bin/get_hcluster_redundance2.pl -type max_length $cds_len_file $dist_file.hcluster  > $dist_file.hcluster.redundance`;
...
`perl $config{"fishInWinter.pl"} -bf table -ff $Format -except  $Outdir/$gff_file_basename.dist.hcluster.redundance $gff_file > $Outdir/$gff_file_basename.nr.gff` if(-e "$Outdir/$gff_file_basename.dist.hcluster.redundance");
`cp $gff_file $Outdir/$gff_file_basename.nr.gff` unless(-e "$Outdir/$gff_file_basename.dist.hcluster.redundance");

`perl $config{"fishInWinter.pl"} -bf table -ff $Format -except  $Outdir/$gff_file_basename.dist.hcluster.all.list $gff_file > $Outdir/$gff_file_basename.uncluster.gff` if(-e "$Outdir/$gff_file_basename.dist.hcluster.all.list");
